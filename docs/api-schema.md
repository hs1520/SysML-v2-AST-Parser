...
| `GET`  | `/api/v1/schema` | Returns current node/edge type catalogue exposed by the controller |
...
| `Interface` | `interface FlightCommandIF { ... }` | – |
| `SystemDef` | `systemdef AutonomousDroneSystem { ... }` | – |
| `DataType` | `datatype MissionPlan { ... }` | – |
| `PartDef` | `part def Vehicle { }` / `partdef Vehicle { }` | `specializations[]`, `specializationKind` |
| `Block` | `block Car { }` | `specializations[]`, `specializationKind` |
| `PartUsage` | `part myCar : Vehicle;` | `type`, `multiplicity` |
| `ComponentUsage` | `component c : C;` | `type` |
| `RequirementDef` | `requirement def PerfReq { }` | `specializations[]`, `specializationKind` |
| `RequirementUsage` | `requirement perf : PerfReq;` | `type` |
| `Subject` | `subject v : Vehicle;` | `type` |
| `PortDef` | `port def EngineIF { }` | `specializations[]`, `specializationKind` |
| `PortUsage` | `in port enginePort : EngineIF;` / `port p : in T;` | `type`, `conjugated` |
| `AttributeDef` | `attribute def Mass { }` | `specializations[]`, `specializationKind` |
| `AttributeUsage` | `attribute mass : Real = 100.0;` | `type`, `defaultValue` |
| `ActionDef` | `action def Start { }` | `specializations[]`, `specializationKind` |
| `ActionUsage` | `action start : Start;` | `type` |
| `StateBehavior` | `state behavior { ... }` | – |
| `State` | `state Navigate { ... }` | – |
| `Connector` | `connect a.p to b.q;` | `source`, `target` |
| `SatisfyRelationship` | `satisfy ReqA, ReqB by partX;` | `requirements[]`, `requirement`, `by` |
| `RefineRelationship` | `refine BaseReq;` | `refines` |
| `Generalization` | `specializes Base;` | `general`, `specializationKind` |
| `Constraint` | `constraint { speed > 100 }` | `expression` |
| `Import` | `import Lib::*;` | `importedNamespace` |
| `Dependency` | `dependency from A to B;` | `from`, `to` |
| `Comment` | `comment about x { /* ... */ }` | `about`, `text` |
| `Documentation` | `doc /* ... */` / `doc "..."` | `text` |
| `MetadataAnnotation` | `metadata #Safety { }` | – |
| `Reference` | Internal unresolved symbol reference node | `unresolved` |

Notes:
- `direction` and `visibility` are top-level `AstNode` fields when applicable, not `properties` entries.
- `Reference` nodes are generated by the parser when relationship targets cannot be resolved to an existing node.
